...
#define bfa_q_enq_q(_dstq, _srcq) {					\
	if (!list_empty(_srcq)) {					\
		bfa_q_next(bfa_q_prev(_srcq)) = (struct list_head *) (_dstq); \
...
#define bfa_q_mv(_srcq, _dstq) {					\
	bfa_assert_fp(list_empty(_dstq));				\
									\
	if (!list_empty(_srcq)) {					\
		bfa_q_next(_dstq) = bfa_q_next(_srcq);			\
		bfa_q_prev(_dstq) = bfa_q_prev(_srcq);			\
		bfa_q_prev(bfa_q_next(_srcq)) = (struct list_head *) _dstq; \
		bfa_q_next(bfa_q_prev(_srcq)) = (struct list_head *) _dstq; \
		INIT_LIST_HEAD(_srcq);					\
	}								\
}

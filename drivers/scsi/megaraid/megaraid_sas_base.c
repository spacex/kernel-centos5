...
megasas_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	instance->verbuf = pci_alloc_consistent(pdev, MEGASAS_MAX_NAME*sizeof(u32),
						&instance->verbuf_h);
	if (!instance->verbuf) {
	...
	sema_init(&instance->reset_mutex, 1);
	...
	if (instance->pdev->device == PCI_DEVICE_ID_LSI_FUSION)
		INIT_WORK(&instance->work_init, megasas_fusion_ocr_wq, instance);
	else
	 ...
	 * Register IRQ
	 */
	if (request_irq(instance->msi_flag ? instance->msixentry.vector : pdev->irq, instance->instancet->service_isr, IRQF_SHARED, "megasas", instance)) {
...
megasas_resume(struct pci_dev *pdev)
{
	 ...
	 * Register IRQ
	 */
	if (request_irq(instance->msi_flag ? instance->msixentry.vector : pdev->irq, instance->instancet->service_isr, IRQF_SHARED,
		"megasas",instance)) {
	...
	switch(instance->pdev->device)
	{
		case PCI_DEVICE_ID_LSI_FUSION:
			megasas_release_fusion(instance);
                        for (i = 0; i < 2 ; i++)
                                if (fusion->ld_map[i])
                                        dma_free_coherent(&instance->pdev->dev, fusion->map_sz, fusion->ld_map[i], fusion->ld_map_phys[i]);
...
megasas_aen_polling(void *arg)
{
	...
	if (doscan) { /* keep this implementation for the later use*/

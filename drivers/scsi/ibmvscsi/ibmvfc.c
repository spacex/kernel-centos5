...
static int ibmvfc_scsilun_to_int(struct scsi_lun *scsilun)
{
	int i;
	unsigned int lun;
...
static void ibmvfc_trc_start(struct ibmvfc_event *evt)
{
	...
	case IBMVFC_CMD_FORMAT:
		...
		entry->lun = ibmvfc_scsilun_to_int(&vfc_cmd->iu.lun);
 ...
 *
 * Returns the FC Explain description text for status returned for
 * Link Services commands as defined in FC-LS-2
 *
 * Returns:
 *	error string
 **/
 ...
 *
 * Returns the FC Explain description text for status returned for
 * Generic Services commands as defined in FC-GS-6
 *
 * Returns:
 *	error string
 **/
...
/**
 * ibmvfc_dma_unmap - Unmap DMA buffer for SCSI command
 * @evt:	ibmvfc event struct
 *
 **/
static void ibmvfc_dma_unmap(struct ibmvfc_event *evt)
{
	struct scsi_cmnd *scmd = evt->cmnd;
	struct ibmvfc_cmd *vfc_cmd = &evt->iu.cmd;
	...
	dma_unmap_single(evt->vhost->dev, data->va, data->len,
...
static void ibmvfc_set_login_info(struct ibmvfc_host *vhost)
{
	...
	struct device_node *of_node = vhost->dev->platform_data;
	const char *location;
			       ...
			       struct srp_direct_buf *md)
{
	...
	for (i = 0, sg = scsi_sglist(scmd); i < nseg; i++, sg++) {
			      ...
			      struct ibmvfc_cmd *vfc_cmd, struct device *dev)
{
	...
	if (scsi_sg_count(scmd)) {
		...
		data->len = scsi_bufflen(scmd);
		data->key = 0;
		data->va = dma_map_single(dev, scmd->request_buffer,
					  data->len, scmd->sc_data_direction);
...
static int ibmvfc_eh_target_reset_handler(struct scsi_cmnd *cmd)
{
	...
	ENTER();
	ibmvfc_wait_while_resetting(vhost);
	rc = ibmvfc_eh_device_reset_handler(cmd);
	if (rc == FAILED) {
...
static ssize_t ibmvfc_show_host_partition_name(struct class_device *class_dev, char *buf)
{
...
static struct class_device_attribute ibmvfc_host_partition_name = {
	.attr = {
		.name = "partition_name",
		.mode = S_IRUGO,
	},
...
static ssize_t ibmvfc_show_host_device_name(struct class_device *class_dev, char *buf)
{
	struct Scsi_Host *shost = class_to_shost(class_dev);
	struct ibmvfc_host *vhost = shost_priv(shost);
...
static struct class_device_attribute ibmvfc_host_device_name = {
	.attr = {
		.name = "device_name",
		.mode = S_IRUGO,
	},
...
static ssize_t ibmvfc_show_host_loc_code(struct class_device *class_dev, char *buf)
{
	struct Scsi_Host *shost = class_to_shost(class_dev);
	struct ibmvfc_host *vhost = shost_priv(shost);
...
static struct class_device_attribute ibmvfc_host_loc_code = {
	.attr = {
		.name = "port_loc_code",
		.mode = S_IRUGO,
	},
...
static ssize_t ibmvfc_show_host_drc_name(struct class_device *class_dev, char *buf)
{
	struct Scsi_Host *shost = class_to_shost(class_dev);
	struct ibmvfc_host *vhost = shost_priv(shost);
...
static struct class_device_attribute ibmvfc_host_drc_name = {
	.attr = {
		.name = "drc_name",
		.mode = S_IRUGO,
	},
...
static ssize_t ibmvfc_show_host_npiv_version(struct class_device *class_dev, char *buf)
{
	struct Scsi_Host *shost = class_to_shost(class_dev);
	struct ibmvfc_host *vhost = shost_priv(shost);
...
static struct class_device_attribute ibmvfc_host_npiv_version = {
	.attr = {
		.name = "npiv_version",
		.mode = S_IRUGO,
	},
 ...
 **/
static ssize_t ibmvfc_show_log_level(struct class_device *class_dev, char *buf)
{
	struct Scsi_Host *shost = class_to_shost(class_dev);
	struct ibmvfc_host *vhost = shost_priv(shost);
	unsigned long flags = 0;
 ...
 * 	number of bytes printed to buffer
 **/
static ssize_t ibmvfc_store_log_level(struct class_device *class_dev,
...
static struct class_device_attribute ibmvfc_log_level_attr = {
	.attr = {
		.name =		"log_level",
...
static struct class_device_attribute *ibmvfc_attrs[] = {
...
static struct scsi_host_template ibmvfc_driver_template = {
	.module = THIS_MODULE,
	.name = "IBM POWER Virtual FC Adapter",
	.proc_name = IBMVFC_NAME,
	.queuecommand = ibmvfc_queuecommand,
	.eh_abort_handler = ibmvfc_eh_abort_handler,
	.eh_device_reset_handler = ibmvfc_eh_target_reset_handler,
...
static void ibmvfc_npiv_login_done(struct ibmvfc_event *evt)
{
	...
	fc_host_port_type(vhost->host) = FC_PORTTYPE_NPORT;
...
static int ibmvfc_probe(struct vio_dev *vdev, const struct vio_device_id *id)
{
	...
	if ((rc = ibmvfc_create_trace_file(&shost->shost_classdev.kobj,
					   &ibmvfc_trace_attr))) {

...
static int pmcraid_slave_configure(struct scsi_device *scsi_dev)
{
	...
	if (RES_IS_GSCSI(res->cfg_entry)) {
		...
		scsi_dev->allow_restart = 1;
		scsi_dev->timeout = PMCRAID_VSET_IO_TIMEOUT;
...
static int pmcraid_notify_aen(struct pmcraid_instance *pinstance, u8 type)
{
	...
	msg_header = genlmsg_put(skb, 0, 0,
				 pmcraid_event_family.id, 0, 0,
				 PMCRAID_AEN_CMD_EVENT,
				 pmcraid_event_family.version);
	if (!msg_header) {
	...
	result =
		genlmsg_multicast(skb, 0, pmcraid_event_family.id);
...
)
{
	...
	for(i=0; i < nseg; i++) {
		ioadl[i].data_len = cpu_to_le32(sg_dma_len(&sglist[i]));
		ioadl[i].address = cpu_to_le64(sg_dma_address(&sglist[i]));
		ioadl[i].flags = 0;
	}
...
static struct pmcraid_sglist *pmcraid_alloc_sglist(int buflen)
{
	...
	memset((scatterlist), 0, sizeof(*scatterlist) * num_elem);
	sglist->order = order;
	...
	for (i = 0; i < num_elem; i++) {
		...
		sglist->scatterlist[i].page = page;
		sglist->scatterlist[i].length = sg_size < bsize_elem ? sg_size : bsize_elem;
		sglist->scatterlist[i].offset = 0;
...
)
{
	...
	/* Initialize IOADL descriptor addresses */
	for(i = 0; i < sglist->num_dma_sg; i++) {
		ioadl[i].data_len = cpu_to_le32(sg_dma_len(&sglist->scatterlist[i]));
		ioadl[i].address = cpu_to_le64(sg_dma_address(&sglist->scatterlist[i]));
		ioadl[i].flags = 0;
	}
...
#define device_attribute class_device_attribute
 ...
 */
static irqreturn_t pmcraid_isr(int irq, void *dev_id, struct pt_regs *regs)
{
...
static void pmcraid_worker_function(void *workp)
{
	struct pmcraid_instance *pinstance;
	struct pmcraid_resource_entry *res;
	...
	pinstance = (struct pmcraid_instance *)workp;
...
)
{
	...
	INIT_WORK(&pinstance->worker_q, pmcraid_worker_function, pinstance);
...
static int pmcraid_setup_chrdev(struct pmcraid_instance *pinstance)
{
	...
	if (error)
		...
		class_device_create(pmcraid_class, NULL, MKDEV(pmcraid_major, minor),
			      NULL, "pmcsas%u", minor);
	return error;
}
...
static void pmcraid_release_chrdev(struct pmcraid_instance *pinstance)
{
	...
	class_device_destroy(pmcraid_class,

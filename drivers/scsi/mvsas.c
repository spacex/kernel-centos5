...
static int mvs_phy_control_wrap(struct asd_sas_phy *sas_phy, enum phy_func func);
...
static void sas_ssp_task_response(struct device *dev, struct sas_task *task,
...
static irqreturn_t mvs_interrupt(int irq, void *opaque,struct pt_regs * pt_regs)
{
...
static int mvs_task_exec(struct sas_task *task, int num, unsigned long gfp_flags)
{
	struct domain_device *dev = task->dev;
...
static struct sas_domain_function_template mvs_transport_ops = {
	...
	.lldd_control_phy	= mvs_phy_control_wrap,
	.lldd_control_phy_new	= mvs_phy_control,
...
MODULE_DESCRIPTION("Marvell 88SE64xx SAS/SATA controller driver");

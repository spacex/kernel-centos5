...
void cxgbi_hbas_remove(struct cxgbi_device *cdev)
{
	...
	for (i = 0; i < cdev->nports; i++) {
		...
		if (chba) {
			cdev->hbas[i] = NULL;
			iscsi2_host_remove(chba->shost);
			pci_dev_put(cdev->pdev);
			iscsi2_host_free(chba->shost);
		}
	}
}
		...
		struct scsi_transport_template *stt)
{
	...
	for (i = 0; i < cdev->nports; i++) {
		shost = iscsi2_host_alloc(sht, sizeof(*chba), 1);
		if (!shost) {
		...
		err = iscsi2_host_add(shost, &cdev->pdev->dev);
		if (err) {
...
static void cxgbi_inform_iscsi_conn_closing(struct cxgbi_sock *csk)
{
	...
	if (csk->state != CTP_ESTABLISHED) {
		...
		if (csk->user_data)
			iscsi2_conn_failure(csk->user_data,
				    ...
				    struct cxgbi_gather_list *gl)
{
	...
	for (i = 0; i < gl->nelem; i++) {
		gl->phys_addr[i] = dma_map_page(&pdev->dev, gl->pages[i], 0,
						PAGE_SIZE,
						PCI_DMA_FROMDEVICE);
		if (unlikely(dma_mapping_error(gl->phys_addr[i]))) {
			log_debug(1 << CXGBI_DBG_DDP,
	...
	if (sc &&
		...
		if (err < 0)
			log_debug(1 << CXGBI_DBG_DDP,
				...
				cconn->cep->csk, task,
				scsi_bufflen(sc), scsi_sg_count(task->sc));
	}
			      ...
			      unsigned int count)
{
	...
	if (task->sc && scsi_sg_count(task->sc)) {
		struct scatterlist *sg = NULL;
		int err;

		tdata->offset = offset;
		tdata->count = count;
		err = sgl_seek_offset(scsi_sglist(task->sc),
				      scsi_sg_count(task->sc),
				      tdata->offset, &tdata->sgoffset, &sg);
		if (err < 0) {
			pr_warn("tpdu, sgl %u, bad offset %u/%u.\n",
				scsi_sg_count(task->sc), tdata->offset,
				scsi_bufflen(task->sc));
			return err;
		}
						...
						u32 initial_cmdsn)
{
	...
	cls_session = iscsi2_session_setup(chba->cdev->itp, shost,
					cmds_max, 0,
...
int cxgbi_iscsi_init(struct iscsi_transport *itp,
			struct scsi_transport_template **stt)
{
	*stt = iscsi2_register_transport(itp);
	if (*stt == NULL) {
			...
			struct scsi_transport_template **stt)
{
	if (*stt) {
		log_debug(1 << CXGBI_DBG_ISCSI,
			...
			itp, itp->name, *stt);
		*stt = NULL;
		iscsi2_unregister_transport(itp);
	}
}
EXPORT_SYMBOL_GPL(cxgbi_iscsi_cleanup);

static void cxgbi_reset_session(struct iscsi_cls_session *cls_session)
{
 ...
 */
int cxgbi_host_reset(struct scsi_cmnd *sc)
{
	iscsi2_host_for_each_session(sc->device->host, cxgbi_reset_session);
	return SUCCESS;
}
EXPORT_SYMBOL_GPL(cxgbi_host_reset);

...
#define to_acpi_device(n) container_of(n, struct acpi_device, dev)
...
static int read_domain_devices(struct acpi_power_meter_resource *resource)
{
	...
	kobject_set_name(&resource->holders_dir, "measures");
	resource->holders_dir.ktype = NULL;
	resource->holders_dir.kset = NULL;
	resource->holders_dir.parent = &resource->acpi_dev->dev.kobj;
...
static int acpi_power_meter_resume(struct acpi_device *device, int state)
{
	struct acpi_power_meter_resource *resource;
...
static int __init enable_cap_knobs(struct dmi_system_id *d)
{
	cap_in_hardware = 1;
	return 0;
}

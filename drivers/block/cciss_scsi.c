...
static void scsi_cmd_stack_free(ctlr_info_t *h)
{
	...
	if (stk->top != stk->nelems-1) {
		dev_err(&h->pdev->dev,
			"%d scsi commands are still outstanding.\n",
			stk->nelems - stk->top);
		/* BUG(); */
		dev_err(&h->pdev->dev, "WE HAVE A BUG HERE!!! stk=0x%p\n", stk);
	}
	size = sizeof(struct cciss_scsi_cmd_stack_elem_t) * stk->nelems;
...
print_bytes (unsigned char *c, int len, int hex, int ascii)
{
	...
	if (hex)
	{
		...
		for (i=0;i<len;i++)
		{
			if ((i % xm) == 0 && i>0) printk("\n");
			if ((i % xm) == 0) printk("0x%04x:", i);
			printk(" %02x", *x);
			x++;
		}
		printk("\n");
...
print_cmd(CommandList_struct *cp)
{
	...
	printk("LUN:0x%02x%02x%02x%02x%02x%02x%02x%02x "
		...
		cp->Request.Timeout);
	printk("CDBLen:%d CDB: %02x %02x %02x %02x %02x %02x %02x %02x"
	...
	printk("edesc.Addr: 0x%08x/0%08x, Len  = %d\n",
		cp->ErrDesc.Addr.upper, cp->ErrDesc.Addr.lower,
			cp->ErrDesc.Len);
	printk("scsistatus: 0x%02x, senselen: %d\n",
		cp->err_info->ScsiStatus, cp->err_info->SenseLen);
	printk("cmd stat:0x%02x scsi stat:0x%02x resid:%d\n",
		cp->err_info->CommandStatus,
		cp->err_info->ScsiStatus,
		cp->err_info->ResidualCnt);
	/* If this is a check condition print the SenseInfo */
	if ((cp->err_info->ScsiStatus & 0x0F) == 0x02) {
		printk("sensedata:\n");
		print_bytes(cp->err_info->SenseInfo,
			cp->err_info->SenseLen, 1, 0);
	}
	printk("offense (sz: %d bt:%d val:0x%02x)\n",
	...
	struct cciss_scsi_dev_t sd[], int nsds)
{
	...
	while (i < ccissscsi[h->ctlr].ndevices) {
		...
		if (found == 0) { /* device no longer present. */
			...
			changes++;
			dev_info(&h->pdev->dev,
				"device c%db%dt%dl%d type changed "
				"(device type now %s).\n",
			...
			cciss_scsi_remove_entry(h, hostno, i,
				removed, &nremoved);
			/* remove ^^^, hence i not incremented */
			if (cciss_scsi_add_entry(h, hostno,
	...
	for (i = 0; i < nsds; i++) {
		...
		if (!found) {
			...
			changes++;
			dev_warn(&h->pdev->dev,
				"device unexpectedly changed type\n");
...
static void
complete_scsi_command(CommandList_struct *c, int timeout, __u32 tag)
{
	struct scsi_cmnd *cmd;
	ctlr_info_t *h;
	ErrorInfo_struct *ei;
	...
	cciss_scatter_gather_unmap(h, c, cmd);
	...
	get interpreted as a missing target. */
			...
			case CMD_ABORTED:
				...
				dev_warn(&h->pdev->dev, "c %p was "
					"aborted\n", c);
			break;
			case CMD_ABORT_FAILED:
				cmd->result = DID_ERROR << 16;
				dev_warn(&h->pdev->dev, "c %p reports "
					"abort failed\n", c);
			break;
			case CMD_UNSOLICITED_ABORT:
				cmd->result = DID_ABORT << 16;
				dev_warn(&h->pdev->dev, "c %p aborted "
					"do to an unsolicited abort\n", c);
			break;
			case CMD_TIMEOUT:
				cmd->result = DID_TIME_OUT << 16;
				dev_warn(&h->pdev->dev, "c %p timedout\n",
					c);
			break;
			...
			default:
				cmd->result = DID_ERROR << 16;
				dev_warn(&h->pdev->dev, "c %p returned "
					"unknown status %x\n", c,
						ei->CommandStatus);
		}
	}
	/* printk("c:%p:c%db%dt%dl%d ", cmd, h->ctlr, cmd->channel,
	 *	cmd->target, cmd->lun);
	 */
	cmd->scsi_done(cmd);
	scsi_cmd_free(h, c);
	...
	switch(ei->CommandStatus)
	{
		...
		case CMD_TIMEOUT:
			dev_warn(&h->pdev->dev, "c %p timedout\n",
				c);
		case CMD_UNABORTABLE:
			dev_warn(&h->pdev->dev, "c %p command unabortable\n",
				c);
		break;
		default:
			dev_warn(&h->pdev->dev, "c %p returned "
...
#define OBDR_TAPE_SIG "$DR-10"
	...
	for(i=0; i<num_luns; i++)
	{
		...
		if (cciss_scsi_do_inquiry(h, scsi3addr, inq_buff,
...
static int
cciss_engage_scsi(ctlr_info_t *h)
{
	struct cciss_scsi_adapter_data_t *sa;
	struct cciss_scsi_cmd_stack_t *stk;
	unsigned long flags;

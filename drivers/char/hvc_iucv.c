...
#define QUEUE_SNDBUF_DELAY	(HZ / 25)
	struct work_struct	sndbuf_work;	/* work: send iucv msg(s) */
...
static unsigned long hvc_iucv_devices = 0;
 ...
 */
static void hvc_iucv_sndbuf_work(void *data)
{
	struct hvc_iucv_private *priv;

	priv = (struct hvc_iucv_private *) data;
	if (!priv)
		return;
...
static void flush_sndbuf_sync(struct hvc_iucv_private *priv)
{
	...
	cancel_delayed_work(&priv->sndbuf_work);
...
static int __init hvc_iucv_alloc(int id, unsigned int is_console)
{
	...
	INIT_WORK(&priv->sndbuf_work, hvc_iucv_sndbuf_work, priv);
...
{
	hvc_iucv_devices = simple_strtoul(val, NULL, 10);
	return 0;
}

/**
 * Backport: core_param() is not yet available, use module_param_named()
 * instead and provide a function for __setup()
 */
static int __init hvc_iucv_allow_config(char *val)
{
	return param_set_vmidfilter(val, NULL);
}


device_initcall(hvc_iucv_init);
__setup("hvc_iucv=", hvc_iucv_config);
__setup("hvc_iucv_allow=", hvc_iucv_allow_config);
module_param_named(hvc_iucv_allow, hvc_iucv_filter, vmidfilter, 0640);

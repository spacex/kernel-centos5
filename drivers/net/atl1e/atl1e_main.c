...
static void atl1e_reset_task(struct net_device *netdev)
{
 ...
 */
static void atl1e_link_chg_task(struct net_device *netdev)
{
	struct atl1e_adapter *adapter = netdev_priv(netdev);
	unsigned long flags;
 ...
 */
static irqreturn_t atl1e_intr(int irq, void *data, struct pt_regs *regs)
{
...
{
	struct atl1e_adapter *adapter = poll_dev->priv;
	struct net_device *netdev  = adapter->netdev;
	u32 imr_data;
	...
	atl1e_clean_rx_irq(adapter, 0, &work_done, work_to_do);
...
static void atl1e_netpoll(struct net_device *netdev)
{
	...
	atl1e_intr(adapter->pdev->irq, netdev, NULL);
				 ...
				 const struct pci_device_id *ent)
{
	...
	INIT_WORK(&adapter->reset_task,
		(void (*)(void *))atl1e_reset_task, netdev);
	INIT_WORK(&adapter->link_chg_task,
		(void (*)(void *))atl1e_link_chg_task, netdev);

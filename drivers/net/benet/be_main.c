...
static unsigned int multi_rxq = 1;
module_param(multi_rxq, uint, S_IRUGO | S_IWUSR);
...
static irqreturn_t be_msix_rx(int irq, void *dev, struct pt_regs *regs)
{
...
static irqreturn_t be_msix_tx_mcc(int irq, void *dev, struct pt_regs *regs)
{
	struct be_adapter *adapter = dev;
...
static int be_poll_rx(struct napi_struct *napi, int budget)
{
	...
	if (work_done < budget) {
		napi_gro_flush(napi);
		netif_rx_complete(rx_eq->napi_dev);
...
static int be_poll_rx_compat(struct net_device *dummy_dev, int *budget)
{
	struct napi_struct *napi = dummy_dev->priv;
	u32 work_done, can_do;

	can_do = min(*budget, dummy_dev->quota);
	work_done = be_poll_rx(napi, can_do);

	*budget -= work_done;
	dummy_dev->quota -= work_done;
...
static int be_poll_tx_mcc_compat(struct net_device *dummy_dev, int *budget)
{
	struct napi_struct *napi = dummy_dev->priv;
	u32 work_done, can_do;

	can_do = min(*budget, dummy_dev->quota);
	work_done = be_poll_tx_mcc(napi, can_do);
...
static int be_close(struct net_device *netdev)
{
	...
	for_all_rx_queues(adapter, rxo, i)
		netif_poll_disable(rxo->rx_eq.napi_dev);

	netif_poll_disable(tx_eq->napi_dev);
...
static int be_open(struct net_device *netdev)
{
	...
	for_all_rx_queues(adapter, rxo, i) {
		be_post_rx_frags(rxo, GFP_KERNEL);
		netif_poll_enable(rxo->rx_eq.napi_dev);
	}
	netif_poll_enable(tx_eq->napi_dev);
...
static int be_netdev_init(struct net_device *netdev)
{
	...
	netdev->features |= NETIF_F_SG | NETIF_F_HW_VLAN_RX | NETIF_F_TSO |
		...
		NETIF_F_IP_CSUM | NETIF_F_GRO | NETIF_F_TSO6;
	...
	for_all_rx_queues(adapter, rxo, i) {
		rxo->rx_eq.napi_dev = alloc_netdev(0, "", ether_setup);
		if (!rxo->rx_eq.napi_dev)
			goto error;
		rxo->rx_eq.napi_dev->priv = &rxo->rx_eq.napi;
		rxo->rx_eq.napi_dev->weight = BE_NAPI_WEIGHT;
		rxo->rx_eq.napi_dev->poll = be_poll_rx_compat;
		rxo->rx_eq.napi.dev = netdev;
		set_bit(__LINK_STATE_START, &rxo->rx_eq.napi_dev->state);
	}

	adapter->tx_eq.napi_dev = alloc_netdev(0, "", ether_setup);
	if (!adapter->tx_eq.napi_dev)
		goto error;
	adapter->tx_eq.napi_dev->priv = &adapter->tx_eq.napi;
	adapter->tx_eq.napi_dev->weight = BE_NAPI_WEIGHT;
	adapter->tx_eq.napi_dev->poll = be_poll_tx_mcc_compat;
	set_bit(__LINK_STATE_START, &adapter->tx_eq.napi_dev->state);
	adapter->tx_eq.napi.dev = netdev;

	return 0;
error:
	for_all_rx_queues(adapter, rxo, i)
		if (rxo->rx_eq.napi_dev) {
			free_netdev(rxo->rx_eq.napi_dev);
			rxo->rx_eq.napi_dev = NULL;
		}
	return -ENOMEM;
}
			...
			const struct pci_device_id *pdev_id)
{
	...
	adapter->rx_obj = kzalloc(sizeof(struct be_rx_obj) * (MAX_RSS_QS + 1),
				  GFP_KERNEL);
	if (!adapter->rx_obj) {
		status = -ENOMEM;
		goto free_netdev;
	...
	return 0;
remove_sysfs_grp:
	sysfs_remove_group(&adapter->netdev->class_dev.kobj,
			   &benet_attr_group);
unreg_netdev:

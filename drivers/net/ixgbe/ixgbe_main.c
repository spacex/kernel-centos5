			       ...
			       struct ixgbe_ring *tx_ring)
{
	...
	if (check_for_tx_hang(tx_ring) && ixgbe_check_tx_hang(tx_ring)) {
		...
		e_info(probe, "tx hang %d detected, resetting adapter\n",
		...
		if (ring_is_ps_enabled(rx_ring)) {
			...
			if (!bi->page_dma) {
				...
				bi->page_dma = dma_map_page(rx_ring->dev,
							    ...
							    PAGE_SIZE / 2,
							    DMA_FROM_DEVICE);
				if (dma_mapping_error(bi->page_dma)) {
					rx_ring->rx_stats.alloc_rx_page_failed++;
					bi->page_dma = 0;
					goto no_buffers;
				}
 ...
 **/
static void ixgbe_check_overtemp_task(struct ixgbe_adapter *adapter)
{
	struct ixgbe_hw *hw = &adapter->hw;
...
static int __ixgbe_shutdown(struct pci_dev *pdev, bool *enable_wake)
{
	...
	case ixgbe_mac_82599EB:
	case ixgbe_mac_X540:
		pci_enable_wake(pdev, PCI_D3hot, !!wufc);
		pci_enable_wake(pdev, PCI_D3cold, !!wufc);
		break;
	default:
		break;
	}
...
void ixgbe_update_stats(struct ixgbe_adapter *adapter)
{
	...
	adapter->net_stats.multicast = hwstats->mprc;

	/* Rx Errors */
	adapter->net_stats.rx_errors = hwstats->crcerrs + hwstats->rlec;
	adapter->net_stats.rx_dropped = 0;
	adapter->net_stats.rx_length_errors = hwstats->rlec;
	adapter->net_stats.rx_crc_errors = hwstats->crcerrs;
			...
			unsigned int first, const u8 hdr_len)
{
	...
	while (len) {
		...
		tx_buffer_info->dma = dma_map_single(dev,
						     skb->data + offset,
						     size, DMA_TO_DEVICE);
		if (dma_mapping_error(tx_buffer_info->dma))
			goto dma_error;
		tx_buffer_info->time_stamp = jiffies;
		tx_buffer_info->next_to_watch = i;
...
static int __ixgbe_maybe_stop_tx(struct ixgbe_ring *tx_ring, int size)
{
	...
	netif_start_queue(tx_ring->netdev);
	++tx_ring->tx_stats.restart_queue;
	return 0;
}
...
int ixgbe_xmit_frame_ring(struct sk_buff *skb,
				 ...
				 const struct pci_device_id *ent)
{
	...
	netdev->features |= NETIF_F_VLAN_SG;
	...
	if (pci_using_dac) {
		...
		netdev->features |= NETIF_F_VLAN_HIGHDMA;
	}

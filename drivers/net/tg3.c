...
static int tg3_load_5701_a0_firmware_fix(struct tg3 *tp)
{
	...
	if (i >= 5) {
		netdev_err(tp->dev, "%s fails to set RX CPU PC, is %08x "
			   "should be %08x\n", __func__,
			   tr32(RX_CPU_BASE + CPU_PC), TG3_FW_TEXT_ADDR);
		return -ENODEV;
	}
	tw32(RX_CPU_BASE + CPU_STATE, 0xffffffff);
...
static int tg3_load_tso_firmware(struct tg3 *tp)
{
	...
	if (i >= 5) {
		netdev_err(tp->dev,
			   ...
			   __func__, tr32(cpu_base + CPU_PC), info.text_base);
		return -ENODEV;
	}
	tw32(cpu_base + CPU_STATE, 0xffffffff);
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	if (tp->tg3_flags2 & TG3_FLG2_5750_PLUS) {
		...
		if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5719)
			tp->tg3_flags &= ~TG3_FLG2_HW_TSO_2;
	}
				  ...
				  const struct pci_device_id *ent)
{
	...
	netdev_info(dev, "%s: Tigon3 [partno(%s) rev %04x PHY(%s)] (%s) %s Ethernet ",
	       dev->name,
	       tp->board_part_number,
	       tp->pci_chip_rev_id,
	       tg3_phy_string(tp),
	       tg3_bus_string(tp, str),
	       ((tp->phy_flags & TG3_PHYFLG_10_100_ONLY) ? "10/100Base-TX" :
		((tp->phy_flags & TG3_PHYFLG_ANY_SERDES) ? "1000Base-SX" :
		 "10/100/1000Base-T")));

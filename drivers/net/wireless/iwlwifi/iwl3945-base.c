				   ...
				   u8 sta_id)
{
	...
	* in uCode. */
	...
	if (priv->stations[sta_id].sta.key.key_offset == WEP_INVALID_OFFSET)
		printk(KERN_WARNING "no space for a new key");
...
#else
	int txq_id = ieee80211_get_queue_mapping(skb);
#endif
...
#else
		iwl_stop_queue(priv, ieee80211_get_queue_mapping(skb));
#endif
	}
...
static void iwl3945_rfkill_poll(void *p)
{
	struct iwl_priv *priv = p;
...
static void iwl3945_setup_deferred_work(struct iwl_priv *priv)
{
	...
	INIT_WORK(&priv->rfkill_poll, iwl3945_rfkill_poll, priv);

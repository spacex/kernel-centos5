...
static irqreturn_t e1000_intr_msix(int irq, void *data)
{
	...
	if (adapter->msix_entries) {
		...
		e1000_intr_msix_rx(msix_irq, netdev, NULL);
		enable_irq(msix_irq);

		vector++;
		msix_irq = adapter->msix_entries[vector].vector;
		disable_irq(msix_irq);
		e1000_intr_msix_tx(msix_irq, netdev, NULL);
		enable_irq(msix_irq);

		vector++;
		msix_irq = adapter->msix_entries[vector].vector;
		disable_irq(msix_irq);
		e1000_msix_other(msix_irq, netdev, NULL);
		enable_irq(msix_irq);
	}
...
static void e1000_netpoll(struct net_device *netdev)
{
	...
	case E1000E_INT_MODE_MSI:
		...
		e1000_intr_msi(adapter->pdev->irq, netdev, NULL);
				 ...
				 const struct pci_device_id *ent)
{
	...
	INIT_WORK(&adapter->led_blink_task,
		(void (*)(void *))e1000e_led_blink_task, adapter);

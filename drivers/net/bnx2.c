			  ...
			  int work_done, int budget)
{
	...
	if (bnx2_get_hw_rx_cons(bnapi) != rxr->rx_cons)
		work_done += bnx2_rx_int(bp, bnapi, min(budget,work_done));
...
gunzip_nomem1:
	pr_err("%s: Cannot allocate firmware buffer for "
			    "uncompression.\n", bp->dev->name);
	return -ENOMEM;
}
...
bnx2_gunzip(struct bnx2 *bp, u8 *zbuf, int len, void **outbuf, int *outlen)
{
	...
	*outbuf = bp->gunzip_buf;
	...
	if ((rc != Z_OK) && (rc != Z_STREAM_END))
		pr_err("%s: Firmware decompression error: %s\n",
...
bnx2_init_cpus(struct bnx2 *bp)
{
	...
	rc = bnx2_gunzip(bp, rv2p, rv2p_len, &text,
			 &text_len);
	if (rc)
		goto init_cpu_err;
...
static struct net_device_stats* bnx2_get_stats(struct net_device *dev)
{
	struct bnx2 *bp = netdev_priv(dev);

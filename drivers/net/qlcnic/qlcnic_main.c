...
static int qlcnic_lro = 1;
module_param(qlcnic_lro, int, 0444);
MODULE_PARM_DESC(qlcnic_lro, "Configure LRO (0=disabled, 1=enabled");
		...
		struct qlcnic_esw_func_cfg *esw_cfg)
{
	...
	features = (NETIF_F_SG | NETIF_F_IP_CSUM |
			NETIF_F_HW_CSUM | NETIF_F_GRO | NETIF_F_VLAN_CSUM);

	if (adapter->capabilities & QLCNIC_FW_CAPABILITY_TSO) {
		features |= (NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_VLAN_TSO);
	}
	if (adapter->capabilities & QLCNIC_FW_CAPABILITY_HW_LRO && qlcnic_lro)
		...
		struct net_device *netdev, u8 pci_using_dac)
{
	...
	netdev->vlan_rx_register   = qlcnic_vlan_rx_register;
	...
	netdev->features |= (NETIF_F_SG | NETIF_F_IP_CSUM | NETIF_F_HW_CSUM |
			NETIF_F_GRO | NETIF_F_VLAN_CSUM | NETIF_F_HW_VLAN_RX |
			NETIF_F_VLAN_SG);
...
static ssize_t
qlcnic_sysfs_get_port_stats(struct kobject *kobj,
		char *buf, loff_t offset, size_t size)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct qlcnic_adapter *adapter = dev_get_drvdata(dev);
...
static ssize_t
qlcnic_sysfs_get_esw_stats(struct kobject *kobj,
		char *buf, loff_t offset, size_t size)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct qlcnic_adapter *adapter = dev_get_drvdata(dev);
...
static ssize_t
qlcnic_sysfs_clear_esw_stats(struct kobject *kobj,
		char *buf, loff_t offset, size_t size)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct qlcnic_adapter *adapter = dev_get_drvdata(dev);
...
static ssize_t
qlcnic_sysfs_clear_port_stats(struct kobject *kobj,
		char *buf, loff_t offset, size_t size)
{
...
qlcnic_create_diag_entries(struct qlcnic_adapter *adapter)
{
	...
	if (sysfs_create_bin_file(&dev->kobj, &bin_attr_port_stats))
	...
	if (sysfs_create_bin_file(&dev->kobj, &bin_attr_esw_stats))
...
qlcnic_remove_diag_entries(struct qlcnic_adapter *adapter)
{
	...
	sysfs_remove_bin_file(&dev->kobj, &bin_attr_port_stats);
	...
	sysfs_remove_bin_file(&dev->kobj, &bin_attr_esw_stats);
}

...
/*		BNX2X_ERR("spurious slowpath interrupt!\n"); */
	...
	if (status & BNX2X_DEF_SB_IDX) {
		...
		if ((!NO_FCOE(bp)) &&
			...
			netif_rx_schedule(&bnx2x_fcoe(bp, dummy_netdev));
#endif
...
#define bnx2x_fw_concat2(a, b, c, d, e) bnx2x_##a##_##b##_##c##_##d##_##e##_fw
#define bnx2x_fw_concat1(a, b, c, d, e) bnx2x_fw_concat2(a, b, c, d, e)
#define bnx2x_fw_concat(a) bnx2x_fw_concat1(a,\
				BCM_5710_FW_MAJOR_VERSION,\
				BCM_5710_FW_MINOR_VERSION,\
				BCM_5710_FW_REVISION_VERSION,\
				BCM_5710_FW_ENGINEERING_VERSION)

static int bnx2x_request_firmware(const struct firmware **firmware_p,
				  const char *name, struct device *device)
{
	static struct firmware fw;
	if (strcmp(name, FW_FILE_NAME_E1) == 0) {
		fw.data = bnx2x_fw_concat(e1);
		fw.size = sizeof(bnx2x_fw_concat(e1));
	} else if (strcmp(name, FW_FILE_NAME_E1H) == 0) {
		fw.data = bnx2x_fw_concat(e1h);
		fw.size = sizeof(bnx2x_fw_concat(e1h));
	} else if (strcmp(name, FW_FILE_NAME_E2) == 0) {
		fw.data = bnx2x_fw_concat(e2);
		fw.size = sizeof(bnx2x_fw_concat(e2));
	} else
		return -ENOENT;
...
#ifdef BCM_CNIC
	/* disable FCOE L2 queue for RHEL5.7 */
	bp->flags |= NO_FCOE_FLAG;
...
#if defined(BCM_CNIC) && defined(NETDEV_HW_ADDR_T_SAN)
	if (!NO_FCOE(bp)) {

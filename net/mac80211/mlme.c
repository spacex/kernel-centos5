...
#else
		  round_jiffies(jiffies + IEEE80211_BEACON_LOSS_TIME));
#endif
}
...
static void ieee80211_chswitch_work(void *s)
{
...
#else
		  round_jiffies(jiffies + IEEE80211_CONNECTION_IDLE_TIME));
#endif
}
					 ...
					 struct sk_buff *skb)
{
	...
	if (ifmgd->associated &&
		...
		case RX_MGMT_CFG80211_DISASSOC:
			...
			break;
		default:
			printk(KERN_WARNING "unexpected: %d", rma);
			WARN_ON(1);
		}
		goto out;
	}
...
static void ieee80211_sta_work(void *s)
{
	struct ieee80211_sub_if_data *sdata = s;
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_if_managed *ifmgd;
	 ...
	 */
	if (local->suspended) {
		printk(KERN_WARNING "STA MLME work scheduled while "
			"going to suspend\n");
		WARN_ON(1);
		return;
	}
...
#else
		if (time_before(jiffies, ifmgd->probe_timeout))
#endif
			run_again(ifmgd, ifmgd->probe_timeout);
	...
	list_for_each_entry_safe(wk, tmp, &free_work, list) {
		...
		case RX_MGMT_CFG80211_ASSOC_TO:
			...
			break;
		default:
			printk(KERN_WARNING "unexpected: %d", wk->tries);
			WARN_ON(1);
		}
...
static void ieee80211_sta_monitor_work(void *s)
{
	struct ieee80211_sub_if_data *sdata = s;
...
void ieee80211_sta_setup_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	ifmgd = &sdata->u.mgd;
	INIT_WORK(&ifmgd->work, ieee80211_sta_work, sdata);
	INIT_WORK(&ifmgd->monitor_work, ieee80211_sta_monitor_work, sdata);
	INIT_WORK(&ifmgd->chswitch_work, ieee80211_chswitch_work, sdata);
	INIT_WORK(&ifmgd->beacon_loss_work, ieee80211_beacon_loss_work, sdata);

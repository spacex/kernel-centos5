...
int ip6addrlbl_newdel(struct sk_buff *skb, struct nlmsghdr *nlh, void *arg)
{
	...
	if (ifal->ifal_index &&
	    !__dev_get_by_index(ifal->ifal_index))
		return -EINVAL;
...
int ip6addrlbl_dump(struct sk_buff *skb, struct netlink_callback *cb)
{
	struct ip6addrlbl_entry *p;
	struct hlist_node *pos;
...
int ip6addrlbl_get(struct sk_buff *in_skb, struct nlmsghdr* nlh, void *arg)
{
	struct ifaddrlblmsg *ifal;
	struct nlattr *tb[IFAL_MAX+1];
	...
	err = nlmsg_unicast(rtnl, skb, NETLINK_CB(in_skb).pid);
out:
	return err;
}

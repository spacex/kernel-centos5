...
static bool is_valid_rd(const struct ieee80211_regdomain *rd)
{
	...
	if (rd->n_reg_rules > NL80211_MAX_SUPP_REG_RULES) {
		WARN_ON(1);
		return false;
	}
...
static void reg_todo(void *unused)
{
	reg_process_pending_hints();
	reg_process_pending_beacon_hints();
}

static DECLARE_WORK(reg_work, reg_todo, NULL);
			...
			u32 country_ie_checksum)
{
	 ...
	 * on the device
	 */
	if (!country_ie_integrity_changes(country_ie_checksum)) {
		WARN_ON(1);
		return true;
	}
	return false;
			...
			u8 country_ie_len)
{
	 ...
	 */
	if (reg_same_country_ie_hint(wiphy, checksum)) {
		WARN_ON(1);
		goto free_rd_out;
	}

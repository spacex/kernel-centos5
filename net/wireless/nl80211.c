...
static struct nla_policy txq_params_policy[NL80211_TXQ_ATTR_MAX + 1] = {
				...
				u8 *mac_addr, struct station_info *sinfo)
{
	...
	if (sinfo->filled & STATION_INFO_TX_BITRATE) {
		...
		if (sinfo->txrate.flags & RATE_INFO_FLAGS_40_MHZ_WIDTH)
			NLA_PUT_FLAG(msg, NL80211_RATE_INFO_40_MHZ_WIDTH, 0);
		if (sinfo->txrate.flags & RATE_INFO_FLAGS_SHORT_GI)
			NLA_PUT_FLAG(msg, NL80211_RATE_INFO_SHORT_GI, 0);
...
#else
	netdev = __dev_get_by_index(ifidx);
#endif
	if (!netdev) {
		err = -ENODEV;
		goto out_rtnl;
...
#else
	genlmsg_multicast(msg, 0, NL80211_GROUP_SCAN);
#endif
}
...
void nl80211_send_reg_change_event(struct regulatory_request *request)
{
	...
	genlmsg_multicast_allns(msg, 0, nl80211_regulatory_mcgrp.id,
				GFP_ATOMIC);
#else
	genlmsg_multicast(msg, 0, NL80211_GROUP_REGULATORY);
#endif
	rcu_read_unlock();
...
#else
	genlmsg_multicast(msg, 0, NL80211_GROUP_MLME);
#endif
	return;
				      ...
				      const u8 *addr, gfp_t gfp)
{
	...
	NLA_PUT_U32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, netdev->ifindex);
	NLA_PUT_FLAG(msg, NL80211_ATTR_TIMED_OUT, 0);
			       ...
			       const u8 *ie, size_t ie_len, bool from_ap)
{
	...
	if (from_ap)
		NLA_PUT_FLAG(msg, NL80211_ATTR_DISCONNECTED_BY_AP, 0);
	if (ie)

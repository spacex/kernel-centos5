...
static int setkey(struct ncrypto_tfm *parent, const u8 *key,
		  unsigned int keylen)
{
	struct priv *ctx = ncrypto_tfm_ctx(parent);
...
static int init_tfm(struct ncrypto_tfm *tfm)
{
	struct crypto_cipher *cipher;
	struct crypto_instance *inst = (void *)tfm->__crt_alg;
	const char *spawn = crypto_instance_ctx(inst);
	struct priv *ctx = ncrypto_tfm_ctx(tfm);
...
static void exit_tfm(struct ncrypto_tfm *tfm)
{
	struct priv *ctx = ncrypto_tfm_ctx(tfm);
	crypto_free_cipher(ctx->child);
...
static struct crypto_instance *alloc(struct rtattr **tb)
{
	...
	inst = ocrypto_alloc_instance("xts", alg);
	if (IS_ERR(inst))
		goto out_put_alg;

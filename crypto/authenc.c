				 ...
				 unsigned int flags)
{
	...
	nscatterwalk_map_and_copy(hash, cipher, cryptlen,
				 ...
				 unsigned int cryptlen)
{
	...
	nscatterwalk_map_and_copy(ihash, cipher, cryptlen, authsize, 0);
...
static int crypto_authenc_init_tfm(struct ncrypto_tfm *tfm)
{
	struct crypto_instance *inst = (void *)tfm->__crt_alg;
	struct authenc_instance_ctx *ictx = crypto_instance_ctx(inst);
	struct crypto_authenc_ctx *ctx = ncrypto_tfm_ctx(tfm);
	struct crypto_hash *auth;
	struct crypto_ablkcipher *enc;
	int err;
...
static void crypto_authenc_exit_tfm(struct ncrypto_tfm *tfm)
{
	struct crypto_authenc_ctx *ctx = ncrypto_tfm_ctx(tfm);
...
static struct crypto_instance *crypto_authenc_alloc(struct rtattr **tb)
{
	...
	struct ncrypto_alg *auth;
	struct ncrypto_alg *enc;
	...
	auth = crypto_attr_alg(tb[1], NCRYPTO_ALG_TYPE_HASH,
	...
	err = crypto_init_spawn(&ctx->auth, auth, inst, NCRYPTO_ALG_TYPE_MASK);
	if (err)
		goto err_free_inst;
	...
	inst->alg.cra_flags |= enc->cra_flags & NCRYPTO_ALG_ASYNC;
	...
	inst->alg.cra_aead.maxauthsize = auth->cra_hash.digestsize;
